...
class _LoginPage extends State<LoginPage> {
  ...
  Future<void> handleSignInGoogle() async {
    try {
      ...
      if (authResult.additionalUserInfo.isNewUser) {
        _newUserPage(authResult.user?.uid);
      } else {
        _existingUserPage(authResult.user?.uid);
      }
    } catch (e) {}
  }
    ...
    _auth.signInWithEmailAndPassword(email: em, password: pw).then((value) {
      ...
      if (value.user.isEmailVerified) {
        if (value.additionalUserInfo.isNewUser) {
          _newUserPage(value.user?.uid);
        } else {
          _existingUserPage(value.user?.uid);
        }
      } else {
        showCupertinoDialog(
            context: context,
    ...
    if (result.status == FacebookLoginStatus.loggedIn) {
      ...
      _auth.signInWithCredential(credential).then((value) {
        Navigator.pushReplacement(
            context,
            MaterialPageRoute(
                builder: (context) => MyContacts(userUID: value.user?.uid)));
      }).catchError((err) {
        print(err.toString());
      });
    }
  ...
  void _existingUserPage(String uid) {
    //Navigate to New User Page
    Navigator.pushReplacement(
        context,
        MaterialPageRoute(
